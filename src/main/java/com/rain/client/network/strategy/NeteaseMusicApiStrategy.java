 ...
 * @since 1.6
 */
public class NeteaseMusicApiStrategy extends AbstractMusicApiStrategy {
    ...
    public NeteaseMusicApiStrategy() {
        this.apiBaseUrl = ModConfig.NETEASE_SEARCH_API_URL;
        MusicPlayerClientMod.LOGGER.info("NeteaseMusicApiStrategy 初始化 - API基础URL: {}", apiBaseUrl);
    }
    ...
    public String getStrategyName() {
        return MusicStrategyEnum.NETEASE.getPlatform();
    }
    ...
    private Map<String, String> parseTrackDetailResponse(String responseBody) {
        try {
            ...
            for (int i = 0; i < data.length(); i++) {
                ...
                String url = obj.optString("url", null), id = obj.optString("id", null);
                resMap.put(id, url);
            }
            return resMap;
        } catch (Exception e) {
    ...
    private Lyric parseLyricResponse(String responseBody) {
        try {
            ...
            // 解析歌词
            List<Lyric.LyricLine> lines = this.parseLrc(Map.of("content", lrcContent)),
                    translationLines = this.parseLrc(Map.of("content", translationContent));
            return new Lyric(lines, translationLines);
        } catch (Exception e) {
